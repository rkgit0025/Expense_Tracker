...
export default function Section5_HotelExpenses({ rows, onChange, readOnly }) {
  ...
  // Changing the room-sharing count resizes sharing_with to (sharing - 1)
  // slots — one per person besides the claimant sharing the room — keeping
  // existing entries where they still fit and trimming/padding as needed.
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <span className="card-title">Hotel Expenses</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: '12px' }}>
            ...
            <div className="form-group" style={{ margin: 0 }}>
              ...
              <select className="form-select" value={row.sharing} disabled={readOnly}
                ...
                {[1, 2, 3, 4].map(n => <option key={n} value={n}>{n === 1 ? 'Single' : n === 2 ? 'Double' : n === 3 ? 'Triple' : 'Quad'} ({n})</option>)}
              </select>
            </div>
            <div className="form-group" style={{ margin: 0 }}>
              <label className="form-label">Hotel / City</label>
              <input type="text" className="form-control" placeholder="Hotel name / City"
            ...
            <div className="form-group" style={{ margin: 0, gridColumn: 'span 2' }}>
              ...
              <input type="text" className="form-control" placeholder="Any notes about this stay (optional)"
          ...
          {extraPeopleNeeded > 0 && (
            <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px dashed var(--gray-200)' }}>
              <label className="form-label" style={{ marginBottom: '8px', display: 'block' }}>
                Sharing Room With <span className="required">*</span> ({extraPeopleNeeded} {extraPeopleNeeded === 1 ? 'person' : 'people'}, besides yourself)
              </label>
              ...
              {!readOnly && extraPeopleNeeded > 0 && (
                ...
                <div style={{ fontSize: '12px', color: 'var(--amber)', fontWeight: 500, marginTop: '2px' }}>
                  ⚠ Every person sharing the room must be identified before this can be submitted.
                </div>
              )}
            </div>
          )}
      ...
      {!readOnly && (
        <button className="add-row-btn" onClick={addRow}>＋ Add Hotel Entry</button>
      )}
    </div>
  );
}
